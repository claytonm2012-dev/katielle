...
/* =========================
   ALUNO: Modal + Carrossel (mesmo sem vídeo)
========================= */
function bindModal(){
...
function renderStudentWelcome(name){
  ...
  if(text) text.textContent = `Comece por aqui e depois explore os grupos abaixo. Use a busca para achar exercícios.`;
}
...
function videoCardHTML(ex){
  ...
  const badge = playable ? `<span class="badge-ok">PLAY</span>` : `<span class="badge-miss">SEM VÍDEO</span>`;
  return `
...
function renderStudentVideos(){
  ...
  // Comece por aqui: mostra os com vídeo primeiro
  const start = list.filter(ex => !!youtubeToEmbed(ex.youtube || "")).slice(0, 12).map(videoCardHTML);
